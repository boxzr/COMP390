...
GLfloat mat_diffuse[] = {0.75164, 0.60648, 0.22658, 1.0};
GLfloat mat_specular[] = {0.628281, 0.555802, 0.366065, 1.0};
GLfloat mat_shininess[] = {128.0};  // Higher shininess for more focused highlights
...
GLfloat viewer_position[] = {0.0, 0.0, 100.0};
...
Vector3D reflectVector(Vector3D incident, Vector3D normal) {
    Vector3D result;
    float dot = dotProduct(incident, normal);
    result.x = incident.x - 2.0f * dot * normal.x;
    result.y = incident.y - 2.0f * dot * normal.y;
    result.z = incident.z - 2.0f * dot * normal.z;
    return result;
}
...
Color calculateLightingAtPoint(Point3D point, Vector3D normal) {
    ...
    if (diffuseFactor > 0.0f) {
        ...
        Vector3D reflectVector = {
            2.0f * diffuseFactor * normal.x - lightVector.x,
            2.0f * diffuseFactor * normal.y - lightVector.y,
            2.0f * diffuseFactor * normal.z - lightVector.z
        };
        reflectVector = normalize(reflectVector);
        
        float specularFactor = dotProduct(reflectVector, viewVector);
        if (specularFactor > 0.0f) {
            specularFactor = pow(specularFactor, mat_shininess[0]);
...
void drawCustomLitSphere(float radius, int slices, int stacks) {
    for (int i = 0; i < stacks; ++i) {
        ...
        for (int j = 0; j <= slices; ++j) {
            ...
            Point3D p1 = {
                radius * sin(phi1) * cos(theta),
                radius * cos(phi1),
                radius * sin(phi1) * sin(theta)
            };
            
            Point3D p2 = {
                radius * sin(phi2) * cos(theta),
                radius * cos(phi2),
                radius * sin(phi2) * sin(theta)
            };
