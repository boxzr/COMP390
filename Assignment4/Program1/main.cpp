/**
 * Assignment 4: Program 1 - Recursive Reflection
 * 
 * This program implements recursive ray tracing to simulate reflections.
 * Based on the recursive algorithm in unit 10 section 2 objective 3.
 * 
 * The render() method has been rewritten to implement the recursive ray tracing algorithm.
 * When a ray hits the floor (reflective surface), the method recursively casts 
 * a reflection ray and calculates a mixed color.
 */
...
Color trace(const Ray& ray, int depth) {
    ...
    // If no intersection, return black (background color)
    ...
    result_color = ambient * surface_color + diffuse_color;
    ...
    if (intersect_result.surface == FLOOR && depth < MAX_DEPTH) {
        ...
        // Create the reflection ray (offset slightly to avoid self-intersection)
        ...
        // Mix the local color with the reflection color
        result_color = result_color * (1 - REFLECTION_COEFFICIENT) + 
...
int main(int argc, char** argv) {
    ...
    glutCreateWindow("Ray Tracing - Recursive Reflection");
